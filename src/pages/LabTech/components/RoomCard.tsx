...
export default function RoomCard({ code, type, status, nextSchedule, currentUser, onViewDetails }: RoomCardProps) {
    ...
    return (
        <div className="bg-gray-800 rounded-xl p-5 border border-gray-700 hover:border-indigo-500 transition-all duration-200 flex flex-col">
            {/* Header */}
                ...
                {isAvailable ? (
                    nextSchedule && (
                        <div className="text-sm text-gray-400">
                            ...
                            <span className="ml-2 text-gray-300">{nextSchedule}</span>
                        </div>
                    )
                ) : (
                    <div className="text-sm text-gray-400 space-y-1">
                        <div>
                            <span className="text-gray-300">{nextSchedule || '—'}</span>
                        </div>
                        {currentUser && (
                            <div className="text-right text-gray-400">{currentUser}</div>
                        )}
                    </div>
                )}
            </div>
