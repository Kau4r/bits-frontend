interface RoomCardProps {
    code: string;
    type: string;
    status: 'Available' | 'In Use' | 'Maintenance' | 'Reserved' | 'Closed';
    nextSchedule?: string;
    currentUser?: string;
...
export default function RoomCard({
    ...
    const showCurrentUser = Boolean(
        normalizedCurrentUser && normalizedCurrentUser.toLowerCase() !== status.toLowerCase()
    );
                ...
                {showScheduleInfo ? (
                    isAvailable ? (
                        ...
                        <div className="text-sm text-gray-500 dark:text-gray-400 space-y-1">
                            ...
                            {showCurrentUser && (
                                <div className="text-right text-gray-500 dark:text-gray-400">{normalizedCurrentUser}</div>
                            )}
                        </div>
                    ) : null
                ) : (
