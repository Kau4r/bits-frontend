...
import type { Computer as ServiceComputer } from '@/services/computer';
...
export const ComputerSelector: React.FC = () => {
  ...
  const handleSelect = () => {
    ...
    const serviceComputer = computers.find(c => c.Computer_ID === parseInt(selectedId));
    if (serviceComputer) {
        ...
        >
          ...
          {computers.map((computer) => (
            <option key={computer.Computer_ID} value={computer.Computer_ID}>
              {computer.Name} - {computer.Room?.Name || 'Unknown Room'}
            </option>
          ))}
        </select>
      </div>
